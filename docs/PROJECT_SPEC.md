...
분석 조건에서 기간 프리셋과 직접 날짜 입력을 제공한다. 조건 변경은 변경하지 않은
기간·필터·비교 기준을 보존하며, 사용자가 직접 바꾼 조건을 원래 질문보다 우선한다.
조회 근거에서 실제 집계·비교 기간, 누락된 조회와 경고를 확인하고 현재 분석 데이터를
CSV로 내려받을 수 있다. CSV는 화면에서 숨긴 위젯의 Dataset도 포함하며 전체 집계와
분류별 집계를 구분한다. 빈 값은 0으로 바꾸지 않는다.

자동 정리는 숨김과 표시 형식을 유지하고 배치만 정리한다. 숨긴 위젯은 각각 복구할
수 있으며 초기화와 구분한다. 버전 기록은 분석 결과 아래 접이식 목록으로 제공하고
후속 요청 중 버전 전환을 막는다. 자세한 점검 결과와 후속 우선순위는
`PRODUCT_REVIEW.md`를 따른다.
